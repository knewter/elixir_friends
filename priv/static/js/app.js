...
var PostList = React.createClass({
  ...
  subscribeToNewPosts: function subscribeToNewPosts() {
    var _this2 = this;

    var socket = new _depsPhoenixWebStaticJsPhoenix.Socket("/socket");
    socket.connect();
    var channel = socket.channel("posts:new", {});
    channel.join().receive("ok", function (chan) {
      console.log("joined");
    });
    channel.on("new:post", function (post) {
      _this2.injectNewPost(post);
    });
  },
  injectNewPost: function injectNewPost(post) {
    this.setState({
...
var PostedAt = React.createClass({
  ...
  render: function render() {
    var timeSince = moment(this.props.date).fromNow();
